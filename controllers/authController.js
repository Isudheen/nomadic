...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const hi = await user.save({ validateBeforeSave: false });
  //To turn off validation before saving, since this save only involves password reset token and expiry
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({
    ...
    // PasswordResetExpires: { $gt: Date.now() },  //removed temporarily for bug fix
  });
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findById(req.user.id).select('+password'); //currentUser will be in req, since protect will be run before
